...
所有測試／除錯／驗證的書面報告一律寫到 `.claude/development/reports/`，檔名用 `YYYY-MM-DD_主題.md`（例：`2026-05-25_沙盒實測報告.md`）。日後任何實測、除錯、驗證的成果都放這裡，不要散落在 repo 根目錄。Claude 產出此類 report 時應直接存到該資料夾。
